...
static const char *const dmic0_groups[] = {
	"gpio96", "gpio97",
};
...
static const char *const sdc_groups[] = {
	"gpio32", "gpio33",
};
...
static const char *const wlan1_groups[] = {
	"gpio14", "gpio15",
};
...
static const struct pinfunction shikra_functions[] = {
	...
	MSM_PIN_FUNCTION(dmic0),
	MSM_PIN_FUNCTION(dmic1),
	...
	MSM_PIN_FUNCTION(rgmii0),
	MSM_PIN_FUNCTION(rgmii1),
	MSM_PIN_FUNCTION(sd_write_protect),
	MSM_PIN_FUNCTION(sdc),
...
static const struct msm_pingroup shikra_groups[] = {
	...
	[4] = PINGROUP(4, qup0_se1_l2, qup0_se1_l1, ddr_bist, _, phase_flag, dac_calib, _, _, _, _, _),
	[5] = PINGROUP(5, qup0_se1_l3, qup0_se1_l0, _, phase_flag, dac_calib, _, _, _, _, _, _),
	...
	[10] = PINGROUP(10, qup0_se3, qup0_se3, _, _, _, _, _, _, _, _, _),
	[11] = PINGROUP(11, qup0_se3, qup0_se3, _, phase_flag, _, _, _, _, _, _, _),
	[12] = PINGROUP(12, qup0_se4, qup0_se4, char_exec, _, _, _, _, _, _, _, _),
	[13] = PINGROUP(13, qup0_se4, qup0_se4, char_exec, _, _, _, _, _, _, _, _),
	[14] = PINGROUP(14, qup0_se5, pll, tgu_ch_trigout, dac_calib, wlan1, _, _, _, _, _, _),
	[15] = PINGROUP(15, qup0_se5, tgu_ch_trigout, _, dac_calib, wlan1, _, _, _, _, _, _),
	...
	[20] = PINGROUP(20, qup0_se7, qup0_se7, cri_trng, _, _, _, _, _, _, _, _),
	[21] = PINGROUP(21, qup0_se7, qup0_se7, tsense_pwm, _, _, _, _, _, _, _, _),
	...
	[26] = PINGROUP(26, qup0_se9_l2, qup0_se9_l1, _, _, _, _, _, _, _, _, _),
	[27] = PINGROUP(27, qup0_se9_l3, qup0_se9_l0, prng_rosc, _, _, _, _, _, _, _, _),
	[28] = PINGROUP(28, qup0_se1_l2, qup0_se6, emac_mcg, prng_rosc, _, phase_flag, qdss_cti,
					_, _, _, _),
	[29] = PINGROUP(29, qup0_se1_l3, qup0_se6, emac_mcg, _, phase_flag, qdss_cti, _, _, _, _, _),
	[30] = PINGROUP(30, qup0_se2, qup0_se6, _, phase_flag, qdss_cti, _, _, _, _, _, _),
	[31] = PINGROUP(31, qup0_se2, qup0_se6, emac1_ptp, emac1_ptp, _, phase_flag, qdss_cti, _, _,
					_, _),
	[32] = PINGROUP(32, pwm, sdc, _, _, _, _, _, _, _, _, _),
	[33] = PINGROUP(33, emac1_ptp, emac1_ptp, sdc, _, _, _, _, _, _, _, _),
	...
	[48] = PINGROUP(48, _, qup0_se9_l0, _, _, pbs_in, phase_flag, _, _, _, _, _),
	[49] = PINGROUP(49, _, qup0_se9_l1, _, _, pbs_in, phase_flag, _, _, _, _, _),
	[50] = PINGROUP(50, _, qup0_se9_l2, _, _, pbs_in, phase_flag, _, _, _, _, _),
	[51] = PINGROUP(51, _, qup0_se9_l3, pbs_in, _, _, _, _, _, _, _, _),
	...
	[58] = PINGROUP(58, _, nav_gpio, pwm, _, pbs_in, atest_bbrx, atest_usb, vsense_trigger_mirnat,
					emac_dll, _, _),
	[59] = PINGROUP(59, _, vfr_1, _, pbs_in, atest_bbrx, atest_usb, emac_dll, _, _, _, _),
	[60] = PINGROUP(60, _, emac1_ptp, emac1_ptp, emac0_ptp, emac0_ptp, _, pbs_in, atest_gpsadc,
	...
	[68] = PINGROUP(68, _, ssbi_wtr, emac1_ptp, emac1_ptp, pwm, dac_calib, _, _, _, _, _),
	[69] = PINGROUP(69, _, ssbi_wtr, emac0_ptp, emac0_ptp, _, phase_flag, dac_calib, _, _, _, _),
	...
	[96] = PINGROUP(96, dmic0, cam_mclk, i2s1, jitter_bist, atest_gpsadc, atest_usb, _, _, _,
					_, _),
	[97] = PINGROUP(97, dmic0, i2s1, dac_calib, _, _, _, _, _, _, _, _),
	[98] = PINGROUP(98, dmic1, cam_mclk, i2s1, _, _, atest_usb, ddr_pxi, _, _, _, _),
	[99] = PINGROUP(99, dmic1, i2s1, jitter_bist, _, atest_usb, ddr_pxi, _, _, _, _, _),
	[100] = PINGROUP(100, i2s2, nav_gpio, _, _, atest_usb, ddr_pxi,  _, _, _, _, _),
	[101] = PINGROUP(101, i2s2, nav_gpio, _, _, atest_usb, ddr_pxi, _, _, _, _, _),
	...
	[121] = PINGROUP(121, rgmii0, _, _, _, _, _, _, _, _, _, _),
	[122] = PINGROUP(122, rgmii0, _, _, _, _, _, _, _, _, _, _),
	[123] = PINGROUP(123, rgmii0, _, _, _, _, _, _, _, _, _, _),
	[124] = PINGROUP(124, rgmii0, _, _, _, _, _, _, _, _, _, _),
	[125] = PINGROUP(125, rgmii0, _, _, _, _, _, _, _, _, _, _),
	[126] = PINGROUP(126, rgmii0, _, _, _, _, _, _, _, _, _, _),
	[127] = PINGROUP(127, rgmii0, _, _, _, _, _, _, _, _, _, _),
	[128] = PINGROUP(128, rgmii0, _, _, _, _, _, _, _, _, _, _),
	[129] = PINGROUP(129, rgmii0, _, _, _, _, _, _, _, _, _, _),
	[130] = PINGROUP(130, rgmii0, _, _, _, _, _, _, _, _, _, _),
	[131] = PINGROUP(131, rgmii0, _, _, _, _, _, _, _, _, _, _),
	[132] = PINGROUP(132, rgmii0, _, _, _, _, _, _, _, _, _, _),
	[133] = PINGROUP(133, rgmii0, _, _, _, _, _, _, _, _, _, _),
	[134] = PINGROUP(134, rgmii0, _, _, _, _, _, _, _, _, _, _),
	[135] = PINGROUP(135, rgmii0, _, _, _, _, _, _, _, _, _, _),
	[136] = PINGROUP(136, emac_phy, _, _, _, _, _, _, _, _, _, _),
	[137] = PINGROUP(137, rgmii1, _, _, _, _, _, _, _, _, _, _),
	[138] = PINGROUP(138, rgmii1, _, _, _, _, _, _, _, _, _, _),
	[139] = PINGROUP(139, rgmii1, _, _, _, _, _, _, _, _, _, _),
	[140] = PINGROUP(140, rgmii1, _, _, _, _, _, _, _, _, _, _),
	[141] = PINGROUP(141, rgmii1, _, _, _, _, _, _, _, _, _, _),
	[142] = PINGROUP(142, rgmii1, _, _, _, _, _, _, _, _, _, _),
	[143] = PINGROUP(143, rgmii1, _, _, _, _, _, _, _, _, _, _),
	[144] = PINGROUP(144, rgmii1, _, _, _, _, _, _, _, _, _, _),
	[145] = PINGROUP(145, rgmii1, _, _, _, _, _, _, _, _, _, _),
	[146] = PINGROUP(146, rgmii1, _, _, _, _, _, _, _, _, _, _),
	[147] = PINGROUP(147, rgmii1, _, _, _, _, _, _, _, _, _, _),
	[148] = PINGROUP(148, rgmii1, _, _, _, _, _, _, _, _, _, _),
	[149] = PINGROUP(149, rgmii1, _, _, _, _, _, _, _, _, _, _),
	[150] = PINGROUP(150, rgmii1, _, _, _, _, _, _, _, _, _, _),
